...
import { construirMapa, prioridadesSemana } from './mapa-progreso'
...
describe('variantes y mapa accionable', () => {
  ...
  it('cruza etiquetas primarias y secundarias sin duplicar la selección ni las prioridades', () => {
    ...
    const selected = prioridadesSemana(cells).flatMap(p => p.conceptos.map(x => x.concept_id))
    expect(selected).toEqual([c.concept_id])
  })
