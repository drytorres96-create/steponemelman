import { intentoCorrecto, type ProgresoConcepto, type EstadoDominio } from './tipos'
...
export interface CriteriosDominio {
  ...
  exigirRecuperacionActiva: boolean // al menos una por recuperación activa
...
export function evaluarDominio(p: ProgresoConcepto, c: CriteriosDominio, ahora = Date.now()): EvidenciaDominio {
  const correctas = p.intentos.filter(intentoCorrecto)
  ...
  const activa = correctas.some(i => i.recuperacion_activa)
  const confusionReciente = p.intentos.some(i =>
    i.tipo_error === 'confusion_conceptos' && (ahora - i.ts) < c.ventanaConfusionDias * DIA)

  const detalle = [
    { criterio: `${c.recuperaciones} recuperaciones correctas`, cumplido: correctas.length >= c.recuperaciones, valor: `${correctas.length}` },
    ...
    ...(c.exigirRecuperacionActiva ? [{ criterio: 'al menos una por recuperación activa', cumplido: activa, valor: activa ? 'sí' : 'no' }] : []),
...
export function calcularEstado(p: ProgresoConcepto, c: CriteriosDominio, ahora = Date.now()): EstadoDominio {
  ...
  if (p.dominado_en && !intentoCorrecto(ultimo)) return 'reaprendizaje'
  if (ev.cumple) return estaVencido(p, ahora) ? 'requiere_repaso' : 'dominado'
  if (estaVencido(p, ahora)) return 'requiere_repaso'
  const correctas = p.intentos.filter(intentoCorrecto).length
...
export function etapa(p: ProgresoConcepto): Etapa {
  if (!p.intentos.length) return 'exposicion'
  if (p.dominado_en) return 'dominio'
  const correctas = p.intentos.filter(intentoCorrecto).length
